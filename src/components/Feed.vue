  ...
  <div class="feed-list">
      <el-row :gutter="14">
        <el-col v-for="(feed, index) in items" :key="index" :sm="12" :md="12" :lg="8">
          <div class="feed-item" :class="{ 'query-match': query == feed.title }" :style="{ 'background-color': feed.color }">
  ...
  export default {
    ...
    methods: {
      ...
      faveIt(item) {
          ...
          axios.post('/?method_name=like_item&method_args=' + item.link + '&method_args=' + item.title + '&method_args=' + this.isFaved(item.link) + '&user=0')
      },
      faveTime(time) {
